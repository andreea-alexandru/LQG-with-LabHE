...
class PaillierPublicKey(object):
    ...
    def raw_encrypt(self, plaintext, r_value=None):
        ...
        """
        if not isinstance(plaintext, int) and not isinstance(plaintext, type(mpz(1))) and not isinstance(plaintext, np.int64):
...
class PaillierPrivateKey(object):
    ...
    def raw_decrypt(self, ciphertext):
        ...
        """
        # isinstance(scalar, np.int64):
        if not isinstance(ciphertext, int) and not isinstance(ciphertext, type(mpz(1))) and not isinstance(ciphertext, np.int64):
...
class EncryptedNumber(object):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, EncryptedNumber):
            ...
            if isinstance(other, LabHE.LabEncryptedNumber):
                return other + self
            else:
    ...
    def _raw_mul(self, plaintext):
        ...
        if not isinstance(plaintext, int) and not isinstance(plaintext, type(mpz(1))) and not isinstance(plaintext, np.int64):
            raise TypeError('Expected ciphertext to be int, not %s' %
                type(plaintext))
        if isinstance(plaintext, np.int64):
            print('Warning: the plaintext is np.int64 represented only on 64 bits. Turned it to int.')
        ...
        if self.public_key.n - self.public_key.max_int <= plaintext:
            ...
            return gmpy2.powmod(neg_c, neg_scalar, self.public_key.nsquare)
        else:
            return gmpy2.powmod(self.ciphertext(False), plaintext, self.public_key.nsquare)
